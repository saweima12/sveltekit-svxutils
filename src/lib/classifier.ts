...
export interface ClassifierHandle<Locals = Record<string, any>, Result = Record<string, any>> {
  ...
  }) :  Promise<ClassifierHandleResult<Result>>
}


export interface ClassifyHandle {
  ( input : {
    classifierList: Array<ClassifierOptions>,
...
export const classifyPages : ClassifyHandle = async (input) => {
  ...
  classifierList.map(async (_classifierOption: ClassifierOptions ) => {
    ...
    if (_classifierOption.type == "directory")
      _classifier = DirectoryClassifierHandle;
    else if (_classifierOption.type == "frontmatter")
      _classifier = FrontMatterClassifierHandle;
    else
      _classifier = _classifierOption.type;
...
const DirectoryClassifierHandle: ClassifierHandle<DirectoryClassifierParams, DirectoryClassifierResult> = async({options, pages}) => {
  ...
  pages.map((page: SourcePage) => {
    ...
    if (!sourcePath.includes(params.path))
      return;
...
const FrontMatterClassifierHandle: ClassifierHandle<FrontMatterClassifierParams, FrontMatterClassifierResult> = async({options, pages}) => {
  let _classifiedPages = {};
  let { id, params } = options;
...
const _classifierIndexAdd = (
  map: Record<string, any> | Object, 
  key:string, 
  item: any
) => {
